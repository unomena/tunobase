...
class TwitterBackend(ModelBackend):
    ...
    def authenticate(self, twitter_oauth_token=None, twitter_oauth_token_secret=None):
        twitter = Twython(
        ...
        if verification_details:
            try:
                ...
                last_name = ' '.join(verification_details['name'].split(' ')[1:])
                user = get_user_model().objects.create(
                    ...
                    last_name=last_name,
                    city=verification_details['location'] if 'location' in verification_details else None
                )
