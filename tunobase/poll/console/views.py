...
from tunobase.poll.console import forms
...
class PollAnswerFormSetMixin(object):
    ...
    def get_context_data(self, **kwargs):
        context = super(PollAnswerFormSetMixin, self)\
                .get_context_data(**kwargs)

        if self.request.method == 'POST':
            context['formset'] = forms.PollAnswerFormSet(
                self.request.POST, 
...
class PollCreate(AdminMixin, PollAnswerFormSetMixin,
...
class PollUpdate(AdminMixin, PollAnswerFormSetMixin,
...
class PollDetail(AdminMixin, generic_views.DetailView):
    ...
    def get_object(self):
        return get_object_or_404(
                poll_models.PollQuestion, pk=self.kwargs['pk']
        )
